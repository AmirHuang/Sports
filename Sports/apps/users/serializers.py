...
class UserRegSerializer(serializers.ModelSerializer):
    ...
    mobile = serializers.CharField(required=True, allow_blank=False, label='手机号', help_text='手机号',
                                   validators=[UniqueValidator(queryset=User.objects.all(), message='该手机已经注册')],
                                   max_length=11, min_length=11, error_messages={'max_length': '手机格式错误',
                                                                                 'min_length': '手机格式错误'})
    name = serializers.CharField(required=True, allow_blank=False, max_length=10, min_length=2,
                                 error_messages={'max_length': '姓名格式错误，长度应该小于等于4',
                                                 'min_length': '姓名格式错误，长度应该大于等于2'},
                                 label='姓名')
    ...
    def validate_code(self, code):
        ...
        if verify_codes:
            last_verifycode = verify_codes[0]
            five_minute_ago = datetime.now() - timedelta(hours=0, minutes=5, seconds=0)
            if five_minute_ago > last_verifycode.add_time:
                raise serializers.ValidationError('验证码过期')
            if code != last_verifycode.code:
