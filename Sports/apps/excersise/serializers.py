...
from .models import Sports, Schedule, JoinSchedule, Sport_Fav
...
class ScheduleSerializer(serializers.Serializer):
    ...
    address = serializers.CharField(required=True, max_length=25, label='详细地址',
                                    error_messages={'max_length': '详细地址长度不能超过25'})
    now_people = serializers.IntegerField(read_only=True, default=1)
    people_nums = serializers.IntegerField(min_value=2, required=True, max_value=20, label='约定总人数',
                                           error_messages={
                                               'required': '请填写总人数',
                                               'min_value': '人数至少为两人，一起运动更愉快!',
...
class ScheduleSerializerTwo(serializers.ModelSerializer):
    # 约运动序列化
    user = serializers.HiddenField(default=serializers.CurrentUserDefault())
    school = serializers.HiddenField(default=CurrentSchoolDefault())
    # sport = serializers.PrimaryKeyRelatedField(queryset=Sports.objects.all(), required=True, label='运动')
    sport = serializers.SlugRelatedField(slug_field='sport_name', queryset=Sports.objects.all())
    sport_time = serializers.DateTimeField(format='%Y-%m-%d %H:%M:%S', label='运动开始时间')
    sport_end_time = serializers.DateTimeField(format='%Y-%m-%d %H:%M:%S', label='运动结束时间')
    add_time = serializers.DateTimeField(read_only=True, format='%Y-%m-%d %H:%M:%S')
    address = serializers.CharField(required=True, max_length=25, label='详细地址',
...
class ScheduleDetailSerializer(serializers.ModelSerializer):
    ...
    def get_joiners(self, obj):
        ...
        all_joiners_json = JoinScheduleDetailSerializer(all_joiners,
                                                        many=True,
...
class MyJoinScheduleSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if schedules:
            for s in schedules:
                if s.schedule.sport_time > schedule.sport_end_time or s.schedule.sport_end_time < schedule.sport_time:
                    pass
                else:
                    double_status = True
        if double_status:
